...
function initMap(business, latlng) {
  ...
  $.ajax(settings).done(function (response) {
    ...
    results.forEach(function(business) {
      ...
      businessInfo.innerHTML = `<tr><td width="325px;">${businessName} <a href=${businessURL} target="_new">(yelp)</a></td><td width="375px;">${businessAddress}</td><td width="160px;">${businessCat}</td><td width="60px;">${businessPrice} ${businessRating} ${businessReviews}</td><td style="text-align:right; width:70px;">${distance}</td></tr>`;  
