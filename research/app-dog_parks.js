...
function initMap(business, latlng) {
  ...
  var settings = {
    "async": true,
    "crossDomain": true,
    "url": `https://cors-anywhere.herokuapp.com/https://api.yelp.com/v3/businesses/search?&categories=dog_parks&limit=25&radius=20000&sort_by=distance&location=${window.searchText}`,
    "method": "GET",
    "headers": {
  ...
  $.ajax(settings).done(function (response) {
    ...
    results.forEach(function(business) {
      ...
      businessInfo.innerHTML = `<tr><td width="350px;">${businessName} <a href=${businessURL} target="_new">(yelp)</a></td><td width="400px;">${businessAddress}</td><td width="160px;">${businessCat}</td><td width="160px;">${lat}</td><td width="160px;">${long}</td><td style="text-align:right; width:70px;">${distance}</td><td style="font-size:20%;">${businessURL}></td></tr>`; 
